    ...
    await db.language.createMany({
      data: [
        ...
        },
        {
          ...
          code: LANGUAGE.ts,
          template:
            "async function run({ input }: { input: any }): Promise<any> {\n  return null;\n}",
        },
        {
          name: "Python",
          code: LANGUAGE.python,
          template: "def run(input):\n  return None",
        },
        {
          name: "Go",
          code: LANGUAGE.go,
          template:
            "func run(input interface{}) interface{} {\n  return nil\n}",
        },
      ],
    });

    const languages = await db.language
      .findMany({
        select: {
          id: true,
        },
      })
      .then((languages) => languages.map((language) => ({ id: language.id })));
